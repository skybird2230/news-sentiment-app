"""
app.py (root-files version)
----------------------------
Same News Sentiment Panel web app, but adjusted to work when all files
(app.py, manifest.json, service-worker.js, icon-192.png, icon-512.png,
requirements.txt) sit together in the SAME folder -- no "static" subfolder
needed. This matches how GitHub's drag-and-drop uploader puts files by
default.
"""
...
# NOTE: paths below point to /manifest.json, /icon-192.png etc (root),
# NOT /static/... -- because all files live together in one folder.
...
# Serve manifest.json, service-worker.js, icons directly from the same
# folder app.py lives in (root of the repo).
